    ...
    for i_iter in range(n_iter):
        for row_i0, row_i1 in izip(box_idxs[:-1], box_idxs[1:]):
            for col_i0, col_i1 in izip(box_idxs[:-1], box_idxs[1:]):
                ...
                print 'adj_idxs', adj_idxs
                print 'coeffs[adj_idxs]', coeffs[adj_idxs]
                coeffs1, fit_result = fit_adjuster_set(coeffs1, adj_idxs, method)
                fit_results.append(fit_result)
        pickle.dump(fit_results, open('box_neg_fit_results.pkl', 'w'), protocol=-1)
        np.save('box_neg_coeffs.npy', coeffs1)
